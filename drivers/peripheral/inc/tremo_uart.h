...
typedef struct {
    ...
    uart_data_width_t data_width;      /*!< Data width */
    uart_parity_t parity;              /*!< Parity */
    uart_stop_bits_t stop_bits;        /*!< Stop bits */
    uart_flow_control_t flow_control;  /*!< Flow control policy */
...
void uart_set_tx_fifo_threshold(uart_t* uartx, uart_tx_fifo_level_t uart_fifo_level);
void uart_set_rx_fifo_threshold(uart_t* uartx, uart_rx_fifo_level_t uart_fifo_level);
