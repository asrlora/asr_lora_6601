...
bool RegionCN470TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    *txPower = txPowerLimited;
    ...
    LOG_PRINTF(LL_DEBUG, "Tx, Power: %d, Freq: %u, DR: %d, len: %u, duration %llu\r\n",
                phyTxPower, (unsigned int)Channels[txConfig->Channel].Frequency, txConfig->Datarate, txConfig->PktLen, *txTimeOnAir);
    return true;
}
