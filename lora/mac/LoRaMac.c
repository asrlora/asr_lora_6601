...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    if( LoRaMacClassBRxBeacon( payload, size ) == true )
    {
        ...
        LOG_PRINTF(LL_VDEBUG, "receive beacon\r\n");
        return;
    }
    ...
    switch ( macHdr.Bits.MType ) {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            if( LoRaMacConfirmQueueIsCmdActive( MLME_JOIN ) == true )
            {
                if( micRx == mic ) {
                    ...
                    LOG_PRINTF(LL_VDEBUG, "Rx1DrOffset:%u Rx2Channel.Datarate:%u ReceiveDelay1:%u\r\n",
            ...
            if ( isMicOk == true ) {
                ...
                if ( ( ( size - 4 ) - appPayloadStartIndex ) > 0 ) {
                    ...
                    if(!frameLen) {
                        ...
                        LOG_PRINTF(LL_DEBUG, "Invalid frame with only frame port\r\n");
                    }
#endif                    
                    ...
                    if ( port == 0 ) {
                        // Only allow frames which do not have fOpts
                            ...
                            McpsConfirm.AckReceived = false;
                            McpsIndication.AckReceived = false;
                            LOG_PRINTF(LL_VDEBUG, "CMD exist at FRMpayload and Fopts, ignore it\r\n");
                        }
                    } else {
#ifdef CONFIG_LWAN
                        if ( fCtrl.Bits.Adr != AdrCtrlOn ) {
                ...
                LOG_PRINTF(LL_VDEBUG, "MIC verify failed ignore the frame\r\n");
        ...
        default:
            LOG_PRINTF(LL_VDEBUG, "Download frame %d being received but not process it\r\n", macHdr.Bits.MType);
    ...
    if ( LoRaMacFlags.Bits.MacDone == 1 ) {
            ...
            if ( ( LoRaMacFlags.Bits.MlmeReq == 1 ) || ( ( LoRaMacFlags.Bits.McpsReq == 1 ) ) ) {
                if ( ( LoRaMacFlags.Bits.MlmeReq == 1 ) && ( LoRaMacConfirmQueueIsCmdActive( MLME_JOIN ) == true ) ) {
                    ...
                    if( LoRaMacConfirmQueueGetStatus( MLME_JOIN ) == LORAMAC_EVENT_INFO_STATUS_OK ) {
                        ...
                        UpLinkCounter = 0;
                        LOG_PRINTF(LL_VDEBUG, "Join done, UpLinkCounter:%u\r\n", (unsigned int)UpLinkCounter);
                    ...
                    if ( ( ChannelsNbRepCounter >= LoRaMacParams.ChannelsNbRep ) || ( LoRaMacFlags.Bits.McpsInd == 1 ) ) {
                        ...
                        if ( IsUpLinkCounterFixed == false ) {
                            UpLinkCounter++;
                            LOG_PRINTF(LL_VDEBUG, "Unconfirmed data, UpLinkCounter:%u\r\n", (unsigned int)UpLinkCounter);
                        }
        ...
        if ( LoRaMacFlags.Bits.McpsInd == 1 ) {
            ...
            if ( ( McpsConfirm.AckReceived == true ) || ( AckTimeoutRetriesCounter > AckTimeoutRetries ) ) {
                ...
                if ( IsUpLinkCounterFixed == false ) {
                    UpLinkCounter++;
                    LOG_PRINTF(LL_VDEBUG, "Confirmed data received ACK, UpLinkCounter:%u\r\n", (unsigned int)UpLinkCounter);
                }
                McpsConfirm.NbRetries = AckTimeoutRetriesCounter;
        ...
        if ( ( AckTimeoutRetry == true ) && ( ( LoRaMacState & LORAMAC_TX_DELAYED ) == 0 ) ) {
            ...
            if ( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) ) {
                ...
                if ( ScheduleTx( ) == LORAMAC_STATUS_OK ) {
                    ...
                    if ( IsUpLinkCounterFixed == false ) {
                        UpLinkCounter++;
                        LOG_PRINTF(LL_VDEBUG, "Confirmed data can't send after decrease DR, UpLinkCounter:%u\r\n", (unsigned int)UpLinkCounter);
                    }
                }
            } else {
                ...
                if ( IsUpLinkCounterFixed == false ) {
                    UpLinkCounter++;
                    LOG_PRINTF(LL_VDEBUG, "Confirmed data exceed retry times, UpLinkCounter:%u\r\n", (unsigned int)UpLinkCounter);
                }
            }
        }
    }
    ...
    switch ( cmd ) {
        ...
        case MOTE_MAC_DEV_STATUS_ANS:
            if ( MacCommandsBufferIndex < ( bufLen - 2 ) ) {
                ...
                SetMlmeScheduleUplinkIndication( );
                status = LORAMAC_STATUS_OK;
                LOG_PRINTF(LL_VDEBUG, "ready to send MOTE_MAC_DEV_STATUS_ANS p1=%d p2=%d\r\n",p1,p2);
            }
            break;
        ...
        case MOTE_MAC_DEVICE_TIME_REQ:
            if( MacCommandsBufferIndex < LORA_MAC_COMMAND_MAX_LENGTH )
            {
                ...
                // No payload for this answer
                status = LORAMAC_STATUS_OK;
                LOG_PRINTF(LL_VDEBUG, "ready to send MOTE_MAC_DEVICE_TIME_REQ\r\n");
            }
            break;
        case MOTE_MAC_PING_SLOT_INFO_REQ:
            if( MacCommandsBufferIndex < ( LORA_MAC_COMMAND_MAX_LENGTH - 1 ) )
            {
                ...
                MacCommandsBuffer[MacCommandsBufferIndex++] = p1;
                status = LORAMAC_STATUS_OK;
                LOG_PRINTF(LL_VDEBUG, "ready to send MOTE_MAC_PING_SLOT_INFO_REQ value=%d\r\n",p1);
            }
            break;
...
static void ProcessMacCommands( uint8_t *payload, uint8_t macIndex, uint8_t commandsSize, uint8_t snr, LoRaMacRxSlot_t rxSlot )
{
    ...
    while ( macIndex < commandsSize ) {
        ...
        LOG_PRINTF(LL_VDEBUG, "MacCommands:%d being processed\r\n", payload[macIndex]);
        ...
        switch ( payload[macIndex++] ) {
            ...
            case SRV_MAC_DEV_STATUS_REQ: {
                ...
                LOG_PRINTF(LL_VDEBUG, "receive SRV_MAC_DEV_STATUS_REQ\r\n");
            ...
            case SRV_MAC_DEVICE_TIME_ANS:
                {
                    ...
                    LOG_PRINTF(LL_VDEBUG, "receive SRV_MAC_DEVICE_TIME_ANS, set time=%u.%d\r\n", (unsigned int)sysTime.Seconds, sysTime.SubSeconds);
            ...
            case SRV_MAC_PING_SLOT_INFO_ANS:
                {
                    ...
                    {
                        LOG_PRINTF(LL_VDEBUG, "receive SRV_MAC_PING_SLOT_INFO_ANS in RX1\r\n");
            ...
            case SRV_MAC_PING_SLOT_CHANNEL_REQ:
                {
                    ...
                    LOG_PRINTF(LL_VDEBUG, "receive SRV_MAC_PING_SLOT_CHANNEL_REQ frequency=%u datarate=%u\r\n", (unsigned int)frequency, datarate);
            ...
            case SRV_MAC_BEACON_FREQ_REQ:
                {
                    ...
                    frequency |= ( uint32_t )payload[macIndex++] << 16;
                    frequency *= 100;
                    LOG_PRINTF(LL_VDEBUG, "receive SRV_MAC_BEACON_FREQ_REQ frequency=%u\r\n", (unsigned int)frequency);
...
LoRaMacStatus_t Send( LoRaMacHeader_t *macHdr, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    if( LoRaMacDeviceClass == CLASS_B )
    {
        LOG_PRINTF(LL_VDEBUG, "Send class b frame\r\n");        
                              ...
                              uint16_t fBufferSize )
{
    ...
    LOG_PRINTF(LL_VDEBUG, "UpLoad frame %d being processed\r\n", macHdr->Bits.MType);
    ...
    switch ( macHdr->Bits.MType ) {
        case FRAME_TYPE_JOIN_REQ:
            ...
            LOG_PRINTF(LL_VDEBUG, "DevNonce:%d\rn\n", LoRaMacDevNonce);
